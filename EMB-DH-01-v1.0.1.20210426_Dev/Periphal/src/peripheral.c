...
#define ApplicationAddress   0x08012000
...
void TIM3_NVIC_Config(void)
{
  ...
  NVIC_TIM3_InitStructure.NVIC_IRQChannelPreemptionPriority = 2; //1   //2
...
void Decrypt_Function(void)
{
	...
	status = STM32_AES_CTR_Decrypt( (uint8_t *) (0x08010000), PLAINTEXT_LENGTH, Key, IV, sizeof(IV), (uint8_t*)(0x2000ff00),
                            &OutputMessageLength);
  if (status == AES_SUCCESS)
  {
		if(*((uint16_t*)(0x2000ff00)) == 0x3c)
		{
...
{
	uint8_t i=0;
	//static uint8_t Erase_Complete=0;
	...
	/*jump to second part*/
	if(*(volatile uint32_t *)(0x8010010) == 0xFFFFFFFF)
	{		 
...
void Earse_Sec_Part(void)
{
	...
	if(Erase_Complete == 0)
	{
		...
		for(int page_num = 0;page_num < 10;page_num++)
		{
			FLASH_ErasePage(0x8012000 + page_num*0x800);
			Delay_time(1000);	
			FLASH_ClearFlag(FLASH_FLAG_BSY|FLASH_FLAG_EOP|FLASH_FLAG_PGERR);
		}				
		FLASH_Lock();
...
void peripheral_init(void)
{
	...
	TIM3_NVIC_Config();
	TIM3_Config(P_TIMER);  //60     //set 100us
